...
_PDF_DIR_PREFIX = (
    config.PDF_DIR if config.PDF_DIR.endswith("/") else config.PDF_DIR + "/"
)
...
def do_search(query):
    ...
    try:
        ...
        if ranked_rows:
            ...
            first_term = (
                phrase_match.group(1).lower()
                if phrase_match
                else (terms[0] if terms else "")
            )
            ...
            if first_term:
                if first_term in partial_terms:
                    ...
                    _ft_re = re.compile(
                        r"\b" + re.escape(first_term) + r"\b", re.IGNORECASE
                    )
            else:
                _ft_re = None
            ...
            for row in c.fetchall():
                content = html_unescape(
                    (row["content"] or "").replace("\ufffd", "").replace("\f", "")
                )
                pos = 0
...
def index():
    ...
    return render_template(
        "index.html", total_docs=total_docs, site_title=config.SITE_TITLE
    )
...
def stats():
    try:
        ...
        total_size = 0
    return jsonify(
        {"total_documents": total_docs, "total_size": format_size(total_size)}
    )
...
def folders():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT pdf_path FROM documents WHERE pdf_path LIKE ? ESCAPE '\\'",
...
def research_api():
    ...
    for sr in search_results:
        ...
        api_partial_terms = {
            w[1:-1].lower()
            for w in raw_for_partial.split()
            if w.startswith("*") and w.endswith("*") and len(w) > 2
        }
